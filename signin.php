...
// $userinfo = array('Tshegofatso'=>'HRg73KRK', 'Trevor'=>'2pYvgrpk', 'Onika'=>'sRraB9MF');

//JSON User Information
$connectionstring = file_get_contents('./includes/eoifilms.json');
$json = json_decode($connectionstring, true);
...
"</li>".
"<li class='nav-item'>".
"<a class='nav-link' href='custom-invoices.php'>Custom invoices</a>".
"</li>".
"<li class='nav-item'>".
...
"<ul class='navbar-nav ml-auto'>".
"<li class='nav-item active'>".
"<a class='nav-link' href='signin.php'>$userstr<span class='sr-only'>(current)</span></a>".
"</li>".
"<li class='nav-item'>".
...
//PHP Login Verify
//if($userinfo[$_POST['user']] != $_POST['pass'])
//{
//$error = "<script class='error'>
//alert('Username or Password is invalid!');
//</script>";
...
{
    if ($user == $account['UserName'] && $pass == $account['UPassWord'])
	{
		$_SESSION['user'] = $user;
		$_SESSION['pass'] = $pass;
		die("
